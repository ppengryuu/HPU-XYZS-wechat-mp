	...
	<view class="body">
		...
		<template v-else>
			...
			<view v-for="(item,index) in newslist" :key="index"> 
				<index-list-dongtai :item="item" :index="index" :preable="false" :fromBUP="true" :updateUserpic="updateuserpic" :backWhenPostId="fromPost"></index-list-dongtai> 
			</view>  
...
<script> 
	...
	export default {
		...
		onLoad(e) { 
			// 开启监听 
			var info = uni.getStorageSync('userDetail')
			uni.removeStorageSync('userDetail')
			// console.log(info)
			this.userid = info.userid
			this.username = info.username
			this.fromPost = info.fromPost
		...
		onNavigationBarButtonTap(e) {
			if(e.index==0){ 
				uni.showActionSheet({
					itemList:['ta 的资料','举报用户'],
					success: (res) => {
						if(res.tapIndex==1){
							this.reportUser(this.userid)
		...
		methods:{
			...
			async getnewslist(){
				...
				let [err,res] = await this.$http.get('/user/' +this.userid+ '/post/' + this.page);
				// 请求失败处理
				// console.log(res)
			...
			async reportUser(id){
				...
				let [err,res] = await this.$http.post('/user/report/0',{
					id:id,
				},{
					token:true,
					checkToken:true,
