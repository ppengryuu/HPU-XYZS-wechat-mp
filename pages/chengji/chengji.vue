<template>
	<view class="body" :style="{'min-height':screenHeight+'px',width:screenWidth+'px'}">
		...
		<view class="semester-changer u-f-ac">
			<view class="presem u-f-ac" @tap="gopresem" hover-class="hover-a" hover-stay-time="100"><view class="icon iconfont icon-fanhui"></view>上一学期</view>
			<view class="semetername u-f-ajc">{{semsters[semsterscurrent]}}</view>
			<view class="nextsem u-f-ac" @tap="gonextsem" hover-class="hover-a" hover-stay-time="100">下一学期<view class="icon iconfont icon-jinru"></view></view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async getgrade(){
				...
				this.grades = grades
				this.semsterscurrent = semsters.length-1
				return true;
			},
			gotodetail(item,semester){
				// console.log(item)
				uni.setStorageSync("gradeItemInfo", item)
				uni.navigateTo({
					url:'chengji-detail/chengji-detail?semester='+semester
				})
			},
			buttonTap(e){
				if(e.index==0){
					...
					uni.showActionSheet({
						...
						success: (res) => {
							if(res.tapIndex==0){
								uni.navigateTo({
									url:"my-ranking/my-ranking?semster="+this.semsters[this.semsters.length-1]
								})
