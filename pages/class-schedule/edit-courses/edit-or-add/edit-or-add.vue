...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			var startsec = data.section?data.section:"1"
			var endsec = (parseInt(data.length?data.length:"1")+parseInt(startsec)-1).toString()
			this.course_time = this.week[parseInt(data.week?data.week:"1")-1]+' 第'+startsec+'~'+endsec+'节'
		},
		onUnload() {
			if (this.$refs.courseTimePicker.showPicker) {
				this.$refs.courseTimePicker.pickerCancel()
		...
		methods: {
			async submit(){
				...
				if(!this.edited.week){return uni.showToast({title:"上课时间不能为空",icon:"none"})}
				if(!this.edited.section){return uni.showToast({title:"上课时间不能为空",icon:"none"})}
				if(!this.edited.length){return uni.showToast({title:"上课时间不能为空",icon:"none"})}
				uni.showLoading({title:"提交中"})
				console.log('提交数据'+JSON.stringify(this.edited) )
				let [err,res] = await this.$http.post('/user/edittimetable',this.edited,{
					token:true,
					checkToken:true
				});
				// 请求失败处理
			...
			onConfirm(e){
				...
				this.edited.week = e.value.week
				this.edited.section = e.value.section
