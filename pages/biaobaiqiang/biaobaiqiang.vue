<template>
	<view class="body" :style="{'min-height':screenHeight+'px'}">
		<navigation-custom :config="{title:'表白墙',bgcolor:'#f8f8f8',fontcolor:'#0c0c0c',type:3,share:false,menuIcon:[{icon:'../../static/icon/back_.png',text:'返回'}],menuText:'返回'}" @customConduct="publicM.back1()"/>
		<view id='stars' :style="{'opacity':istarshow}"></view>
		<view id='stars2' :style="{'opacity':istarshow}"></view>
		<view id='stars3' :style="{'opacity':istarshow}"></view>

		
		
		<block v-for="(item,index) in bbqlist" :key="index">
			<view class="bbqlist">
				<view class="content">
					<view class="fbdate">{{item.date}}</view>
					<view class="abouto2o">
						<view>{{item.title}}</view>
					</view>
					
					<text class="bbcontent">{{'&emsp;'+item.content}}</text>
				</view>
				...
				<view class="aboutzpz">
					...
					<view v-show="item.infonum.index==0" @tap.stop="switchislike(item)">
						<view class="icon iconfont icon-xihuan1"></view><view class="dianzan">点赞</view><view>{{item.infonum.dingnum}}</view>
					</view>
					<view class="zanle" v-show="item.infonum.index==1" @tap.stop="switchislike(item)">
		...
		<!-- <view class="loadmore" :style="loadmoretext=='上拉加载'?{'opacity':0}:{'opacity':1}">{{loadmoretext}}</view> -->
		<uni-drawer :visible="showRight" mode="right" @close="closeDrawer('right')">
			<uni-section title="搜索" type="line"></uni-section>
			<uni-search-bar radius="5" placeholder="说不定有人提到你" clearButton="auto" cancelButton="none" @confirm="search" />
			<uni-section title="筛选" type="line"></uni-section>
			<view style="display: flex;justify-content: center;">
				<view class="tag-view" style="width: 92%;margin-left: 4%;">
					<view>
						<uni-tag :type="showall?'error':'default'" text="全部" @click="switchshowall" />
					</view>
					<view>
						<uni-tag :type="showliked?'error':'default'" text="我赞过的" @click="switchshowliked" />
					</view>
					<view>
						<uni-tag :type="showbiaobai?'error':'default'" text="表白" @click="switchshowbb" />
					</view>
					<view>
						<uni-tag :type="showzhufu?'error':'default'" text="祝福" @click="switchshowzf" />
					</view>
					<view>
						<uni-tag :type="showtucao?'error':'default'" text="吐槽" @click="switchshowtc" />
					</view>
					<view>
						<uni-tag :type="showxinyuan?'error':'default'" text="心愿" @click="switchshowxy" />
					</view>
					<view>
						<uni-tag :type="showxiaomimi?'error':'default'" text="小秘密" @click="switchshowxmm" />
					</view>
				</view>
			</view>
			<uni-section title="排序方式" type="line"></uni-section>
			<view style="display: flex;justify-content: center;">
				<view class="tag-view" style="width: 92%;margin-left: 4%;">
					<view>
						<uni-tag :type="sortbytime?'error':'default'" text="按时间" @click="switchsortbytime" />
					</view>
					<view>
						<uni-tag :type="sortbyhot?'error':'default'" text="按热度" @click="switchsortbyhot" />
					</view>
					
				</view>
			</view>
			<view class="close">
				<view class="word-btn" hover-class="word-btn--hover" :hover-start-time="20" :hover-stay-time="70" @click="hide"><text class="word-btn-white">取消</text></view>
				<view class="word-btn" style="background: #F76260;" hover-class="word-btn--hover" :hover-start-time="20" :hover-stay-time="70" @click="hide"><text class="word-btn-white">确定</text></view>
			</view>
		</uni-drawer>
...
<script>
	...
	export default {
		...
		onLoad() {
			uni.getSystemInfo({
				success: (res) => {
					this.screenHeight=res.screenHeight-uni.getSystemInfoSync().statusBarHeight-44
				}
			});
			// 开启监听
		...
		methods: {
			...
			async getbbqlist(){
				console.log('调用函数了')
				let url = '/postclass/11/post/' + this.page;
				// 获取顶踩
				...
				this.bbqlist = this.page > 1 ? this.bbqlist.concat(arr) : arr;
				if (list.length < 10) {
			...
			switchshowbb(){
				this.showbiaobai=!this.showbiaobai
				this.showall=0
			},
			switchshowzf(){
				this.showzhufu=!this.showzhufu
				this.showall=0
			},
			switchshowtc(){
				this.showtucao=!this.showtucao
				this.showall=0
			},
			switchshowxy(){
				this.showxinyuan=!this.showxinyuan
				this.showall=0
			},
			switchshowxmm(){
				this.showxiaomimi=!this.showxiaomimi
				this.showall=0
			},
			switchshowliked(){
