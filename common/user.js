...
export default {
			...
			success: (res) => {
				...
				uni.getUserInfo({
				  ...
				  success: async (infoRes)=> {
					...
					let [err1,res1] = await $http.post('/wxlogin',{
						code:res.code,
	...
	async getSchoolBindData(){
		...
		// 获取课程表
		let [err1,res1] = await $http.get('/user/gettimetable',{},{
			token:true,
			checkToken:true,
			checkAuth:true
		});
		...
		// }
		// 获取成绩
		let [err3,res3] = await $http.get('/user/getgrade',{},{
			token:true,
			checkToken:true,
			checkAuth:true
		});
	...
	async getLibBindData(){
		let [err1,res1] = await $http.get('/user/getborrowbook',{},{
			token:true,
			checkToken:true
		});
		// console.log(res1)
